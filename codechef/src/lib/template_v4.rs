...
// template copied from the following user + edits
// https://www.codechef.com/users/unwise

use std::collections::{BinaryHeap, HashMap, HashSet};

fn solve(input: &mut io::input::Input, _test_case: usize) {}

use io::output::out;
use io::output::out_line;

pub mod solution {
    pub fn run(mut input: super::io::input::Input) -> bool {
        #[allow(dead_code)]
        enum TestType {
            Single,
            MultiNumber,
        ...
        match test_type {
            TestType::Single => super::solve(&mut input, 1),
            TestType::MultiNumber => {
                let t = input.read();
                for i in 0usize..t {
                    super::solve(&mut input, i + 1);
                }
            }
            TestType::MultiEof => {
                let mut i = 1usize;
                while input.peek().is_some() {
                    super::solve(&mut input, i);
                    i += 1;
                }
            }
        }
        super::io::output::output().flush();
...
pub mod io {
    pub mod input {
        use super::super::numbers::num_traits::add_sub::AddSub;
        use super::super::numbers::num_traits::from_u8::FromU8;
        use super::super::numbers::num_traits::mul_div_rem::Multable;
        use super::super::numbers::num_traits::sign::IsSigned;
        use super::super::numbers::num_traits::zero_one::ZeroOne;
        use std::fmt::Display;
        use std::io::Read;
    ...
    pub mod output {
        ...
        macro_rules! _out {
            ($first: expr $(,$args:expr )*) => {
                io::output::output().print(&$first);
                $(io::output::output().put(b' ');
                io::output::output().print(&$args);
                )*
                io::output::output().maybe_flush();
            }
        }

        #[macro_export]
        macro_rules! _out_line {
            ($first: expr $(, $args:expr )* ) => {
                io::output::out!($first $(,$args)*);
                io::output::output().put(b'\n');
                io::output::output().maybe_flush();
            };
            () => {
...
fn main() {
    ...
    let input = io::input::Input::new(&mut sin);
    unsafe {
        io::output::OUTPUT = Some(io::output::Output::new(Box::new(std::io::stdout())));
    }
    solution::run(input);
}
