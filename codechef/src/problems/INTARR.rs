...
fn main() {
    for _ in 0..scan!(u32) {
        let n = scan!();
        let mut a = (0..n).map(|_| scan!(usize)).vec();
        a.sort();
        let mut v = vec![];
        a.iter()
            .copied()
            .skip(n / 2)
            .zip(a.iter().copied().take(n / 2))
            .for_each(|e| {
                v.push(e.0);
                v.push(e.1);
            });
        ...
        if v.iter()
            .copied()
            .zip(v.iter().copied().skip(1))
            .zip(v.iter().copied().skip(2))
            .any(|((a, b), c)| a.cmp(&b) == b.cmp(&c))
        {
            sayln!("-1");
        } else {
            v.iter().copied().for_each(|e| {
                say!("{} ", e);
            });
            sayln!();
        }
