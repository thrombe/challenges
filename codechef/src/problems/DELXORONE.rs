...
fn main() {
    for _ in 0..scan!(u32) {
        let n = scan!(u32);
        let mut a = (0..n).map(|_| scan!(u32) >> 1).vec();

        let mut counts = HashMap::new();
        a.iter().for_each(|&e| {
            if let Some(e) = counts.get_mut(&e) {
                *e += 1;
            } else {
                counts.insert(e, 1_u32);
            }
        });

        // dbg!(&counts);
        // let mut c = counts.iter().map(|e| (*e.0, *e.1)).vec();
        // dbg!(&c);
        // c.sort_by(|a, b| a.0.count_ones().cmp(&b.0.count_ones()));
        // // dbg!(&c);
        // if let Some(e) = c.iter().copied().zip(c.iter().copied().skip(1))
        // .filter(|e| e.0.0^e.1.0 <= 1)
        // .map(|e| (e.0.1, e.1.1))
        // .map(|e| e.0 + e.1).max() {
        //     sayln!("{}", n-e);
        // } else {
        //     sayln!("{}", n-c.into_iter().map(|e| e.1).max().unwrap());
        // }

        sayln!("{}", n - counts.values().max().unwrap());
    }
}
